...
def train_test_split(
    ...
    for i in tqdm.tqdm(range(interaction_matrix.shape[0])):
        ...
        test_interactions[i] = [
            index_to_appid[idx] for idx in game_idxs[:user_test_size]
        ]
...
def get_popular_recommendations(
    game_info_df, interaction_matrix, user_index, INDEX_TO_APPID, num_results
):
    game_info_df = game_info_df.copy()
    user_row = interaction_matrix[[user_index], :]

    appids_pop = [
        INDEX_TO_APPID[appid]
        for num, appid in enumerate(list(interaction_matrix[user_index].indices))
    ...
    for appid in appids_pop:
        if not any(
            pd.isnull(game_info_df[game_info_df["AppID"] == appid]["Developers"].values)
        ):
            developers += list(
                game_info_df[game_info_df["AppID"] == appid]["Developers"].values
            )
        if not any(
            pd.isnull(game_info_df[game_info_df["AppID"] == appid]["Categories"].values)
        ):
            categories += list(
                game_info_df[game_info_df["AppID"] == appid]["Categories"].values
            )[0].split(",")
        if not any(
            pd.isnull(game_info_df[game_info_df["AppID"] == appid]["Genres"].values)
        ):
            genres += list(
                game_info_df[game_info_df["AppID"] == appid]["Genres"].values
            )[0].split(",")
    ...
    filtered_df = game_info_df[
        game_info_df.apply(check_values, args=(developers, categories, genres), axis=1)
    ]

    filtered_df["Rating"] = (
        filtered_df["Positive"] / (filtered_df["Positive"] + filtered_df["Negative"])
    ).round(2)
