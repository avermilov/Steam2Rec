...
def describe_perc(arr):
    return pd.Series(arr).describe(percentiles=[0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99])
...
def filter_data(data, lower_q, upper_q):
    left_border, right_border = np.quantile(data, lower_q), np.quantile(data, upper_q)
    
    return data[np.where((data >= left_border) 
                         & (data <= right_border))] 
...
def make_histplot_filtr(feature, data_list, x_labels, rotation=False):
    ...
    filtr = ['no restrictions', 'over 50 reviews', 'over 70 reviews', 'over 100 reviews']
...
def make_countplot_filtr(feature, data_list, x_labels, order, rotation=False, limit = False):
...
def make_distplot(feature, data, x_label, rotation=False, x_min = -math.inf, x_max = math.inf):
...
def make_distplot_filtr(feature, data_list, x_labels, rotation=False, x_min = -math.inf, x_max = math.inf):
    
    fig, axs = plt.subplots(2, 2, figsize=(14, 10))
    filtr = ['no restrictions', 'over 50 reviews', 'over 70 reviews', 'over 100 reviews']
    for num, data in enumerate(data_list):
        ax = axs[num // 2, num % 2]
        sns.distplot(data[feature][(data[feature] > x_min) & (data[feature] < x_max)], ax=ax)
    ...
    def get_feature_counts(self):
        ...
        if self.condition:
            games_feature_counts = Parallel(n_jobs=self.n_jobs)(
                delayed(self.get_file_counts)(game_file_path) for game_file_path in self.game_review_files 
